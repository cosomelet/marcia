...
class Kernels(object):
    ...
    def __init__(self, data, filename = None):
        ...
        if len(self.data) != self.nmodel:
            raise ValueError('Error: data length does not match the number of kernels')
        ...
        for i in range(self.nmodel):
            for j in range(self.nmodel):
                ...
                if i <= j:
                    ...
                    l1_values = np.linspace(MTGP[f'Task_{i+1}']['l_min'], MTGP[f'Task_{i+1}']['l_max'], MTGP['KERNEL']['n_points'])
                    l2_values = np.linspace(MTGP[f'Task_{j+1}']['l_min'], MTGP[f'Task_{j+1}']['l_max'], MTGP['KERNEL']['n_points'])
                    tau_values = np.linspace(-10, 10, MTGP['KERNEL']['n_points'])
